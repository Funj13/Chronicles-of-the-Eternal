...
func processar_perseguindo(delta):
	...
	tocar_animacao("zombie_walk/Walk", velocidade / 1.0)
...
func receber_dano(dano: int, posicao_atacante: Vector3 = Vector3.ZERO):
	...
	DamageManager.exibir_dano(global_position, dano)	
	atualizar_vida_visual()
...
func morrer():
	...
	# Inicia a simulação física no esqueleto
	#var esqueleto = $zombie/GeneralSkeleton/PhysicalBoneSimulator3D 

	#esqueleto.physical_bones_start_simulation()

	#print("💀 Zumbi virou Ragdoll!")
	anim_player.play("zombie_death/Death")
